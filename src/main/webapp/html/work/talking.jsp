...
<head>
    ...
    <script type="text/javascript">
        ...
        $(document).ready(function () {
            $("#SearchTalking").click(function () {
                $("#SearchTalking").css("color", "red");
                var keyword = $("input[name='keyword']").val();
                $.ajax({
                    type: "GET",
                    url: "SearchTalking", // 替换为您的Servlet URL
...
<div class="container">
    <div class="public-nav">您当前的位置：<a href="">工作管理</a>><a href="">谈话记录</a></div>
    <div class="public-content">
        <div class="public-content-header">
            <h3>修改信息</h3>
            <div class="search">
                <form action="../../SearchTalking" method="get">
                    <input type="hidden" name="method" value="SearchTalking" />
                    <input placeholder="请输入..." name="keyword" type="text" value="">
                    <button type="button" id="SearchTalking"></button>
                </form>
            </div>
        ...
        <div class="public-content-cont">
            <table class="public-cont-table">
                ...
                <%
                    ArrayList TalkingAL=(ArrayList)session.getAttribute("TalkingAL");
                    Iterator iter=TalkingAL.iterator();
                    DB db = new DB();
                    while(iter.hasNext()){
                        Stutalk talking=(Stutalk) iter.next();
                %>
                <tr><td><%= talking.getStu_id() %></td>
                    <td><%= db.getName(talking.getStu_id()) %></td>
                    <td><%= talking.getDate() %></td>
                    <td><%= talking.getContent() %></td>
                    <td>
                        <div class="table-fun">
                            <a class="btn btn-primary btn-large modify" href="javascript:;"
                               data-stu_id="<%= talking.getStu_id() %>"
                               data-name="<%= db.getName(talking.getStu_id()) %>"
                               data-date="<%= talking.getDate() %>"
                               data-content="<%= talking.getContent() %>">
                                修改
                            </a>
                        </div>
                    </td>
