    ...
    <script type="text/javascript">
        jQuery(document).ready(function($) {
            $('.modify').click(function(){
                ...
                var self_cmt = $(this).data('self_cmt');
                var cls_adv = $(this).data('cls_adv');

                // 填充表单字段
                $('input[name="stu_id"]').val(stu_id);
                $('input[name="name"]').val(name);
                $('input[name="date"]').val(date);
                $('input[name="self_cmt"]').val(self_cmt);
                $('input[name="cls_adv"]').val(cls_adv);
        ...
        $(document).ready(function() {
            $('.theme-signin input[name="submit"]').click(function() {
                ...
                var self_cmt = $('input[name="self_cmt"]').val();
                var cls_adv = $('input[name="cls_adv"]').val();
        ...
        $(document).ready(function () {
            $("#SearchComment").click(function () {
                $("#SearchComment").css("color", "red");
                var keyword = $("input[name='keyword']").val();
                $.ajax({
                    type: "GET",
                    url: "SearchComment", // 替换为您的Servlet URL
        ...
        jQuery(document).ready(function($) {
            $('.add').click(function(){
                ...
                $('input[name="add_self_cmt"]').val("");
                $('input[name="add_cls_adv"]').val("");
        ...
        $(document).ready(function() {
            $('.theme-add input[name="submit"]').click(function() {
                ...
                var self_cmt = $('input[name="add_self_cmt"]').val();
                var cls_adv = $('input[name="add_cls_adv"]').val();
...
<div class="container">
    <div class="public-nav">您当前的位置：<a href="">学生信息</a>><a href="">学生评语</a></div>
    <div class="public-content">
        <div class="public-content-header">
            <h3>修改信息</h3>
            <div class="search">
                <form action="../../SearchComment" method="get">
                    <input type="hidden" name="method" value="SearchComment" />
                    <input placeholder="请输入..." name="keyword" type="text" value="">
                    <button type="button" id="SearchComment"></button>
                </form>
            </div>
        ...
        <div class="public-content-cont">
            <table class="public-cont-table">
                ...
                <tr>
                    ...
                    <th style="width:20%">学生自评</th>
                    <th style="width:20%">教师评语</th>
                    <th style="width:20%">操作</th>
                </tr>
                <%
                    ArrayList CommentAL=(ArrayList)session.getAttribute("CommentAL");
                    Iterator iter=CommentAL.iterator();
                    DB db = new DB();
                    while(iter.hasNext()){
                        Comment comment=(Comment) iter.next();
                %>
                <tr><td><%= comment.getStu_id() %></td>
                    <td><%= db.getName(comment.getStu_id()) %></td>
                    <td><%= comment.getDate() %></td>
                    <td><%= comment.getSelf_cmt() %></td>
                    <td><%= comment.getCls_adv() %></td>
                    <td>
                        <div class="table-fun">
                            <a class="btn btn-primary btn-large modify" href="javascript:;"
                               data-stu_id="<%= comment.getStu_id() %>"
                               data-name="<%= db.getName(comment.getStu_id()) %>"
                               data-date="<%= comment.getDate() %>"
                               data-self_cmt="<%= comment.getSelf_cmt() %>"
                               data-cls_adv="<%= comment.getCls_adv() %>">
                                修改
                            </a>
                        </div>
                    </td>
    ...
    <div class="theme-popbod dform">
        <form class="theme-signin" name="modifyform">
            <ol>
                ...
                <li><strong>学生自评：</strong><input class="ipt" type="text" name="self_cmt" size="20" /></li>
                <li><strong>教师评语：</strong><input class="ipt" type="text" name="cls_adv" size="20" /></li>
    ...
    <div class="theme-popbod dform">
        <form class="theme-add" name="addform" >
            <ol>
                ...
                <li><strong>学生自评：</strong><input class="ipt" type="text" name="add_self_cmt" size="20" /></li>
                <li><strong>教师评语：</strong><input class="ipt" type="text" name="add_cls_adv" size="20" /></li>
                <li><input class="btn btn-primary" type="button" name="submit" value=" 提交 " /></li>
